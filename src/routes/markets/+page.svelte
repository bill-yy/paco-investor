...
<div class="p-6 space-y-6">
	<header>
		...
		<h1 class="text-3xl font-bold text-[var(--color-text-primary)] mt-1">Mercados globales</h1>
	</header>
	...
	<div class="border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] p-4">
		...
		<div class="grid grid-cols-3 md:grid-cols-6 gap-2">
			{#each sectors as s}
				...
				>
					<div class="text-[11px] text-white/80">{s.name}</div>
					<div class="text-lg font-bold tabular text-white">{(s.change_pct * 100).toFixed(2)}%</div>
				</div>
			{/each}
		</div>
	</div>
